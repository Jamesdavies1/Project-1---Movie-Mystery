...
function directorsGenerator() {
  
  $(".game-card-content").html(null)
  for (i = 0; i < 3; i++) {
    randomDirectorsNames.shift();
  };
  ...
  queryImageMeta().then(function (configResponse) {
    ...
    queryRandomMovie().then(function (randomMovie) {
      ...
      var moviePosterUrl = randomMovie.results[randomNumber].poster_path;
      var moviePoster = baseUrl + imageSize + moviePosterUrl;
      var movieTitle = randomMovie.results[randomNumber].title;
      $(".poster").attr("src", moviePoster);
      $(".movie-title").text(movieTitle);

      queryMovieCreditsById(randomMoviesId).then(function (movieCredits) {
